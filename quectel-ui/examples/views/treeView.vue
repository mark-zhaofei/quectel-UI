    ...
    return {
      ...
      nameAfter: (h, params) => {
        if (params.children && params.children.length) {
          return h('el-tag', { class: 'treeTag', props: { size: 'mini', type: 'success' }})
        }
      },
      nameIconRender: (h, params) => {
        if (params.children && params.children.length) {
          return h('i', { class: 'iconfont el-icon-s-goods color-primary' })
        } else {
          return h('i', { class: 'iconfont el-icon-s-goods color-warning' })
        }
      },
      render: (h, params) => {
        const array = []
